...
from test.lib.wvtest import wvcheck, wvfail, wvmsg, wvpass, wvpasseq, wvpassne, wvstart
import bup.path
...
def verify_trees_match(path1, path2):
    global top
    exr = exo((top + b'/test/cmd/compare-trees', b'-c', path1, path2), check=False)
...
def validate_blob(src_id, dest_id):
    ...
    rmrf(b'restore-src')
    rmrf(b'restore-dest')
    cat_tree = top + b'/test/cmd/git-cat-tree'
