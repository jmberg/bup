...
root_status="$(test/cmd/root-status)" || exit $?

TOP="$(WVPASS pwd)" || exit $?
export PATH="$TOP/test/cmd/bin:$PATH"
tmpdir="$(WVPASS wvmktempdir)" || exit $?
export BUP_DIR="$tmpdir/bup"

# Assume that mvmktempdir will always use the same dir.
timestamp_resolutions="$(test/cmd/ns-timestamp-resolutions "$tmpdir/canary")" \
    || exit $?
...
hardlink-sets()
{
    "$TOP/test/cmd/hardlink-sets" "$@"
}

id-other-than()
{
    "$TOP/test/cmd/id-other-than" "$@"
}
...
test-src-save-restore()
{
    ...
    (
        ...
        WVPASS "$TOP/test/cmd/compare-trees" -c src/ src-restore/src/
...
setup-test-tree()
{
    WVPASS "$TOP/test/cmd/sync-tree" "$TOP/test/sampledata/" "$tmpdir/src/"
    ...
    test/cmd/mksock "$tmpdir"/src/test-socket || true
}
...
(
    ...
    WVFAIL $TOP/test/cmd/compare-trees -c src/var/ src-restore/ > tmp-compare-trees
...
(
    ...
    WVPASS "$TOP/test/cmd/compare-trees" -c src/ src-restore-1/src/
    ...
    WVPASS "$TOP/test/cmd/compare-trees" -c src-restore-1/src/ src-restore-2/src/
...
(
    ...
    unknown_user=$("$TOP"/test/cmd/unknown-owner --user) || exit $?
    WVPASS rm -rf src
    ...
    unknown_group=$("$TOP"/test/cmd/unknown-owner --group) || exit $?
    WVPASS rm -rf src
    current_gidx=$(bup meta -tvvf ../src.meta | grep -e '^gid:') || exit $?
