...
# indexed trees for comparison via test/cmd/subtree-hash.
...
indexed_tree1="$(WVPASS test/cmd/subtree-hash "$tree1" "${indexed_top[@]}" src)" \
    || exit $?
...
indexed_tree2="$(WVPASS test/cmd/subtree-hash "$tree2" "${indexed_top[@]}" src)" \
    || exit $?
...
indexed_tree3="$(WVPASS test/cmd/subtree-hash "$tree3" "${indexed_top[@]}" src)" || exit $?
