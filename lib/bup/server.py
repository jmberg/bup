...
from bup.compat import int_types, hexstr
...
class BaseServer:
    ...
    def _new_packwriter(self):
        """
        Return an object implementing the PackWriter protocol, including
        context manager
        """
...
class BupServer(BaseServer):
    ...
    @contextmanager
    def _send_index(self, name):
        yield git.open_idx(git.repo(b'objects/pack/%s' % name)).map
