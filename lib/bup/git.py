...
class PackIdxList:
    ...
    def exists(self, hash, want_source=False, want_offs=False):
        ...
        for i in range(len(self.packs)):
            ...
            ret = p.exists(hash, want_source=get_src, want_offs=get_offs)
            if ret:
                # reorder so most recently used packs are searched first
                self.packs = [p] + self.packs[:i] + self.packs[i+1:]
                if want_offs and ret.offset is None:
                    with open_idx(os.path.join(self.dir, ret.pack)) as np:
                        ret = np.exists(hash, want_source=want_source, want_offs=True)
                    assert ret
                return ret
        self.do_bloom = True
        return None
...
def walk_object(repo, oidx, stop_at=None, include_data=None):
    """Yield everything reachable from oidx via repo.cat as a WalkItem,
    stopping whenever stop_at(oidx) returns true.  Throw MissingObject
    if a hash encountered is missing from the repository, and don't read
    or return blob content in the data field unless include_data is set.
    """
