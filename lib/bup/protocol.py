...
class Server:
    ...
    def init_session(self, repo_dir=None):
        if self.repo:
            ...
            self.deduplicate_writes = True if dw is None else dw
            debug1('bup server: (%sdeduplicating) serving %s\n'
                   % ('' if self.deduplicate_writes else 'not ',
    ...
    def list_indexes(self, junk):
        assert self.deduplicate_writes is not None
        self.init_session()
        suffix = b'' if self.deduplicate_writes else b' load'
