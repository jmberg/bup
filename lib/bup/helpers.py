...
from bup import io
from bup.compat import argv_bytes, byte_int, nullcontext, pending_raise, bytes_from_byte
...
class ProgressBar:
    ...
    def _bar(self):
        ...
        if total is not None:
            ...
            position = (time.time() - self._start)
            _ROUNDTRIP_TIME = 10
            while position > _ROUNDTRIP_TIME:
                position -= _ROUNDTRIP_TIME
            position = int(width * position / _ROUNDTRIP_TIME * 2)
            if position > width:
