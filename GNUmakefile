...
lint: dev/bup-exec dev/bup-python
	$(call quiet,"LINT","(started)")
	$(Q)./pylint

test: all test/tmp dev/python lint
	$(Q)! bup version  # Ensure we can't test the local bup (cf. dev/shadow-bin)
	$(Q)./bup features
	$(Q)./pytest $(xdist_opt)
...
distcheck: all
	if test yes = $$(dev/python -c "import xdist; print('yes')" 2>/dev/null); then \
	  ($(call notquiet,set -x;) ./pytest $(xdist_opt) -m release;) \
	else \
	  ($(call notquiet,set -x;) ./pytest -m release;) \
	fi
